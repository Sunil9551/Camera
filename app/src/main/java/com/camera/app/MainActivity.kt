...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE_PERMISSIONS) {
            if (allPermissionsGranted()) {
                ...
                Toast.makeText(
                    this,
                    "Camera and Microphone permission ia necessary",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
    ...
    private fun switchMode(newMode: CaptureMode) {
        if (recording != null) {
            Toast.makeText(this, "Change mode after recording stop", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun startCameraForCurrentMode() {
        ...
        cameraProviderFuture.addListener({
            ...
            if (currentMode == CaptureMode.PHOTO) {
                ...
                // High Resolution ON -> device ka max supported 4:3 size
                // High Resolution OFF -> ek maddham/default size (kam file size)
